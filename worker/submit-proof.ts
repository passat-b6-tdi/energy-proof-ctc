...
async function main(): Promise<void> {
  ...
  if (!txHash)
    throw new Error("Usage: npm run demo -- <source-transaction-hash>");
  ...
  const { transaction: sourceTx } = await findSourceTransaction(txHash);
  if (sourceTx.blockNumber === null)
    throw new Error("Source transaction is still pending");

  console.log(`Source transaction: ${explorerUrl(sourceExplorer, txHash)}`);
  console.log(`Source block: ${sourceTx.blockNumber}`);
  console.log(
    `Waiting for Attestcoin attestation for chain key ${chainKey}...`,
  );
  ...
  try {
    ...
    gasLimit =
      21_000n + BigInt(continuityProof.roots.length) * 5_000n + 20_000n;
    console.log(
      `Gas estimation unavailable; using fallback gas limit ${gasLimit}`,
    );
  }
  ...
  console.log(
    `Creditcoin execute transaction: ${explorerUrl(creditcoinExplorer, submission.hash)}`,
  );
  const receipt = await submission.wait();
  if (!receipt || receipt.status !== 1)
    throw new Error("Creditcoin execute transaction failed");
