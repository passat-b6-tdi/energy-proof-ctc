...
async function main(): Promise<void> {
  ...
  if (!Number.isInteger(count) || count < 1 || count > 10 || producers.length === 0) {
    throw new Error('Usage: npm run demo:produce -- [1-10] <producer> [producer ...]');
  }
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const transaction = await meter.recordProduction({wattHours, producer, readingId});
    const receipt = await transaction.wait();
    if (!receipt || receipt.status !== 1) throw new Error(`Transaction failed: ${transaction.hash}`);
    console.log(`${index + 1}/${count} ${transaction.hash} block=${receipt.blockNumber} producer=${producer} wattHours=${wattHours} readingId=${readingId}`);
  }
}
