...
async function main(): Promise<void> {
  ...
  const producers = Array.from({ length: 3 }, () =>
  ...
  producers.forEach((producer, index) =>
    console.log(`${index + 1}. ${producer.address}`),
  );
  ...
  try {
    for (let index = 0; index < producers.length; index += 1) {
      ...
      for (let reading = 0; reading < 3; reading += 1) {
        ...
        const transaction = await producerMeter.recordProduction({
          wattHours,
        ...
        console.log(
          `${producer.address} ${transaction.hash} block=${receipt.blockNumber} wattHours=${wattHours} readingId=${readingId}`,
        );
      }
    }
  } finally {
